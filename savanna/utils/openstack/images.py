...
def _ensure_tags(tags):
    return [tags] if type(tags) == str else tags


class SavannaImage(Image):
    def __init__(self, manager, info, loaded=False):
        info['description'] = info['metadata'].get(PROP_DESCR)
        info['username'] = info['metadata'].get(PROP_USERNAME)
        info['tags'] = [tag for tag in _iter_tags(info['metadata'])]
...
class SavannaImageManager(ImageManager):
    ...
    def set_description(self, image, description=None, username=None):
